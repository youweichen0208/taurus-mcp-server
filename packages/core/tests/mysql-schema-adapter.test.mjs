...
test("mysql adapter samples rows with redaction and large-field exclusion", async () => {
  const { adapter, state } = createAdapterWithDefaultMock();
  const sample = await adapter.sampleRows(makeContext(), "demo", "orders", 2);

  assert.equal(sample.columns.length, 3);
  assert.deepEqual(sample.columns.map((column) => column.name), [
    "id",
    "created_at",
    "phone_number",
  ]);
  assert.deepEqual(sample.rows[0], [1, "2026-01-01 00:00:00", "[REDACTED]"]);
  assert.deepEqual(sample.redactedColumns, ["phone_number"]);
  assert.deepEqual(sample.truncatedColumns, []);
  assert.equal(sample.sampleSize, 2);
  assert.equal(sample.totalRowCount, 2);

  const sampleSql = state.executes.find((entry) => entry.sql.includes("FROM `demo`.`orders`"))?.sql ?? "";
  assert.ok(sampleSql.length > 0);
  assert.match(sampleSql, /SELECT `id`, `created_at`, `phone_number`/);
  assert.doesNotMatch(sampleSql, /`note`/);
});

test("mysql adapter truncates oversized string values in sample rows", async () => {
  const longDescription = "x".repeat(1030);
  ...
  const { pool } = makeMockConnectionPool((sql) => {
    ...
    if (sql.includes("FROM information_schema.TABLES") && sql.includes("LIMIT 1")) {
      return {
        rows: [{ table_rows: 1, table_comment: "orders table" }],
      };
    }
  ...
  assert.deepEqual(sample.columns.map((column) => column.name), ["id", "description"]);
  assert.deepEqual(sample.rows, [[1, expectedTruncated]]);
  assert.deepEqual(sample.truncatedColumns, ["description"]);
  assert.deepEqual(sample.redactedColumns, []);
  assert.equal(sample.sampleSize, 1);
  assert.equal(sample.totalRowCount, 1);
});
