...
function asNormalizedTableName(schema) {
    return `${schema.database}.${schema.table}`.toLowerCase();
}
function matchesTableRef(schema, tableRef) {
...
function resolveTableCandidates(schemaSnapshot, tableRef) {
...
function splitColumnRef(columnRef) {
...
function hasSensitiveColumn(schema, columnName) {
    return (schema.engineHints?.sensitiveColumns.some((sensitive) => sensitive.toLowerCase() === columnName.toLowerCase()) ?? false);
}
...
export function validateSchemaAware(cls, schemaSnapshot) {
    const missingTables = [];
...
export function validateCost(cls, explainSummary) {
    const reasonCodes = [];
    const riskHints = [...(explainSummary.riskHints ?? [])];
    let hasConfirm = false;
    let hasMedium = false;
    const escalateToConfirm = (code, hint) => {
        hasConfirm = true;
    ...
    const escalateToMedium = (code, hint) => {
        hasMedium = true;
        reasonCodes.push(code);
        riskHints.push(hint);
    };
    ...
    if (!explainSummary.indexHitLikely &&
