...
- `diagnose_replication_lag`、`diagnose_storage_pressure` 仍是结构化 scaffold
...
- 验证样例库上的 explain / readonly / mutation 行为是否符合预期
- 补一轮本地多会话锁等待与连接堆积场景，验证 diagnostics 第一刀行为

说明：

- 代码、SQL 资产、测试文件已经准备好
- 本地 SQL 主链路你已经验证过；diagnostics 的真实多会话场景还需要补一轮系统化验证
