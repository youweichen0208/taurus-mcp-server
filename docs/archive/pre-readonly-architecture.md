> **历史快照：本文描述已移除的数据库写入能力，不适用于 0.5.x 只读 Harness。**
...
flowchart TB
  ...
  subgraph Core["taurusdb-core"]
    ...
    DIAG["Diagnosis Layer (current first version)<br/>Scenario-based diagnostics"]:::core
  end
  ...
  subgraph ControlPlane["Control plane / metrics"]
...
#### 4.1.4 诊断 Tool（当前已落地，后续增强）

> 这一组已经进入默认 Tool 面，但仍在持续增强云侧证据、长历史源与多源归并。
> 它们的共同特点是：**默认只读**、**场景化输出**、**联合使用管控面指标 + TaurusDB 内核状态 + SQL 现场**。

当前已落地的轻量 evidence collector：
...
当前第一版已经按分层落地，后续主要是增强云侧证据与 merge 质量：

- `find_top_slow_sql`：已基于 digest ranking / Top SQL / Taurus slow-log external source 做 suspect SQL 发现
- `diagnose_service_latency`：已把 slow SQL / 锁 / 连接三类嫌疑串起来做入口层编排，并接入 CES CPU、内存、连接使用率、慢查询、存储延迟、复制延迟指标做资源压力路由
- `diagnose_db_hotspot`：已接入热点对象聚合，能够按 `scope=sql|table|session` 汇总 SQL、锁等待、processlist 热点
- `diagnose_slow_query`：已基于 `EXPLAIN`、`performance_schema` digest / wait history 做本地可验证诊断，再继续补 Taurus slow-log external source、DAS / Top SQL 与更强的云侧运行时关联
- `diagnose_connection_spike`：已基于 `processlist` 做本地可验证的 evidence-backed 诊断，并接入 CES 连接数、活跃连接数、连接使用率、QPS 指标
- `diagnose_lock_contention`：已接入 InnoDB lock wait snapshot、metadata lock 与 latest deadlock collector 第一版
- `diagnose_replication_lag`：已接入复制状态命令与 CES lag / long transaction / write pressure 指标第一版
- `diagnose_storage_pressure`：已接入 digest / table storage evidence 与 CES 存储指标第一版
...
诊断第一版已经落地，当前重点是继续把证据源做完整、把云侧差异补齐。

1. 场景化诊断链路
   当前已经引入一条诊断层，重点不是“多一个 SQL Tool”，而是把管控面指标、TaurusDB 内核视图和 SQL 现场联合起来做根因分析。
   ...
   - `diagnose_service_latency` / `diagnose_db_hotspot` 已接入症状入口编排并补充 `next_tool_inputs`
   ...
   - `diagnose_connection_spike` 已接入 `show_processlist` 与 CES 连接指标第一版
   - `diagnose_lock_contention` 已接入 InnoDB lock wait snapshot、metadata lock 与 latest deadlock collector 第一版
   - `diagnose_replication_lag` 已接入复制状态命令与 CES lag / long transaction / write pressure 指标第一版
   - `diagnose_storage_pressure` 已接入 digest / table storage evidence 与 CES 存储指标第一版
   - DAS / 全量 SQL / 更长 deadlock history / OS 级磁盘指标仍待后续 collector 落地
