...
- 日常工具优先使用数据库最小权限只读账号。不存在可重新开启通用写工具的环境变量或
  Agent 审批 token；回收站恢复是使用当前会话账号、精确目标约束的唯一例外。
...
`restore_recycle_bin_table` 默认可见，不需要审批密钥文件。如客户完全不需要该工具，可设置
  `TAURUSDB_ENABLE_RECYCLE_BIN_RESTORE=false` 隐藏恢复工具。
- 当前会话账号需要具备 TaurusDB 原生回收站恢复权限；MCP 仍不向 Agent 开放其他写入。
- 自然语言恢复请求会调用 `restore_recycle_bin_table`，预检通过后立即执行，不再生成浏览器审批页面。
- 只允许恢复到明确且不存在的目标表，不允许覆盖，也不提供任意 SQL 输入。
- 恢复后执行只读目标验证；请求、成功或失败均记录目标绑定审计证据。
- 需要强身份认证、双人复核或集中 RBAC 的客户，应关闭该工具并在外部受控渠道恢复。
