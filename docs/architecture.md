...
- `diagnose_replication_lag`：已接入 `SHOW REPLICA STATUS` / `SHOW SLAVE STATUS` 与 CES replication delay、long transaction、write IOPS、write throughput，仍需云端复制拓扑验证
- `diagnose_storage_pressure`：已接入 digest / table storage evidence 与 CES 存储用量、读写延迟、IOPS、吞吐、临时表指标，仍需云端真实指标验证
...
1. 场景化诊断链路
   ...
   - DAS / Top SQL / MDL / deadlock history / OS 级磁盘指标仍待后续 collector 落地
