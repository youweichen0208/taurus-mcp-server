...
必须绑定精确回收站对象和不存在的目标表”。旧通用写入流程测试文档
...
- 默认 tools/list 包含 `analyze_mutation_sql` 和 `restore_recycle_bin_table`，不包含通用写入工具；
...
- `restore_recycle_bin_table` 对错误对象、目标冲突、缺少会话凭据或审计能力均不执行恢复；
  成功后目标只读验证和目标绑定审计完整。
