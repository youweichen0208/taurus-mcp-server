...
export function normalizeExplainRows(result: RawResult): Record<string, unknown>[] {
...
export function summarizeExplainRows(
...
export function buildExplainRecommendations(
  summary: ExplainRiskSummary,
): string[] {
  const recommendations: string[] = [];

  if (summary.fullTableScanLikely) {
    recommendations.push(
      "Consider adding WHERE filters and an index to avoid full table scans.",
    );
  }
  ...
  if (summary.estimatedRows !== null && summary.estimatedRows > 100_000) {
    recommendations.push(
      "Estimated row count is high; consider narrowing predicates or adding selective indexes.",
    );
  }
